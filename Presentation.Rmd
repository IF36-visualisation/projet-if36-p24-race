...
plot_ly(data_femmes, x = ~Games, y = ~Proportion_femmes,name = "La proportion de femmes ", type = 'scatter', mode = 'lines', line = list(color = 'blue')) %>%
...
# Filtrer les données pour inclure uniquement les athlètes féminines
data_females <- data_jo %>%
  filter(Sex == "F")

# Agréger les données par sport pour obtenir le nombre de femmes par sport
females_per_sport <- data_females %>%
  count(Sport)

# Créer un graphique en barres montrant la répartition des femmes par sport
ggplot(females_per_sport, aes(x = reorder(Sport, n), y = n)) +
  geom_bar(stat = "identity", fill = "deeppink") +
  geom_text(aes(label = n), hjust = -0.2, color = "black") +  # Ajouter les valeurs des barres
  coord_flip() +  # Pour une meilleure lisibilité
  labs(title = "Répartition des femmes par sport",
       x = "Sport",
       y = "Nombre de femmes") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 24))
