...
data_PIB <- read_csv('GDP_per_capita.csv')
```
...
## l'existance d'une relation entre le niveau économique d’un pays et ses performances aux Jeux Olympiques

###regarde discord freddy

## Analyse des impacts des caractéristiques physiques sur les résultats  (réalisé par : Jabote Mohamed Amine)
...
La ligne bleue représente la proportion de femmes parmi les athlètes participant aux Jeux Olympiques. Nous pouvons observer une tendance à la hausse de la participation des femmes au fil des éditions, reflétant une évolution positive vers une plus grande égalité des genres dans le sport. Cette augmentation de la proportion de femmes témoigne des progrès réalisés en matière de représentation et de participation des femmes dans le sport de haut niveau.

Nombre de femmes :

La ligne rouge représente le nombre total de femmes participant aux Jeux Olympiques. Bien que la proportion de femmes ait augmenté, le nombre absolu de femmes participant a également augmenté au fil du temps. Cette croissance du nombre de femmes athlètes reflète l'élargissement de l'accès des femmes au sport et l'augmentation des opportunités de participation dans diverses disciplines.

En analysant ces données, il est clair que la participation des femmes aux Jeux Olympiques a considérablement augmenté au cours des dernières décennies, témoignant des progrès réalisés en matière d'égalité des genres dans le sport. Cependant, malgré ces avancées, des disparités persistent encore en termes de représentation et de soutien des athlètes féminines, soulignant la nécessité continue de promouvoir l'égalité des genres et de soutenir la participation des femmes dans toutes les disciplines sportives.




```{r, echo=FALSE}
# Charger les données des Jeux Olympiques
data_jo <- read_csv('athlete_events.csv')
...
p <- plot_ly(medals_by_gender, x = ~Year, y = ~total_medals, color = ~Sex, type = 'bar',
             ...
             hoverinfo = 'text') %>%
  layout(title = "Répartition des médailles par genre au fil du temps",
...
```
#Analyse de la participation des femmes aux Jeux Olympiques par pays : 
...
En conclusion, l'analyse de la répartition des médailles par genre aux Jeux Olympiques met en lumière l'évolution positive de la participation des femmes dans le sport de haut niveau. Alors que les femmes continuent de progresser et de briller sur la scène olympique, il est essentiel de continuer à promouvoir l'égalité des genres et à soutenir la participation des femmes dans toutes les disciplines sportives. Cela garantira une représentation équitable et inclusive des athlètes féminines aux Jeux Olympiques et contribuera à l'avancement de l'égalité des genres dans le monde du sport.
...
data_jo <- data_jo %>%
  filter(Sex == "F") %>%
  group_by(NOC, Year) %>%
  summarise(total_female_athletes = n(), .groups = 'drop')

# Fusionner les données des JO avec les régions NOC
female_participation <- data_jo %>%
  left_join(data_noc_regions, by = "NOC")

# Agréger les données par pays
female_participation_country <- female_participation %>%
  group_by(region) %>%
  summarise(total_female_athletes = sum(total_female_athletes), .groups = 'drop')

# Lire les données des codes de pays ISO
country_codes <- read_csv('https://raw.githubusercontent.com/datasets/country-codes/master/data/country-codes.csv')

# Fusionner les données de participation avec les codes de pays ISO
female_participation_map <- female_participation_country %>%
  left_join(country_codes, by = c("region" = "official_name_en"))

# Filtrer pour les données avec des codes ISO disponibles
female_participation_map <- female_participation_map %>%
  filter(!is.na(`ISO3166-1-Alpha-3`))

# Renommer la colonne pour correspondre à Plotly
female_participation_map <- female_participation_map %>%
  rename(iso_alpha = `ISO3166-1-Alpha-3`)
...
## Analyse de la carte choroplèthe

La carte choroplèthe ci-dessus illustre la participation des femmes aux Jeux Olympiques par pays, en mettant en évidence le nombre total de participantes féminines dans chaque pays. La couleur bleu plus foncées indiquent un nombre plus élevé de femmes participant aux Jeux Olympiques, tandis que les couleurs plus claires indiquent un nombre plus faible de participantes. Cette carte ne prend pas en compte les états-unis et la Russie qui sont les pays les plus dominants aux jeux olympiques et qui envoient un grand nombre de femmes aux jeux olympiques.


Il est indéniable que les pays les plus foncés sur la carte, comme la Chine qui a envoyé 2992 femmes, le Japon avec 2779 femmes, et l'Allemagne en tête de liste avec 4849 femmes aux Jeux olympiques, illustrent un engagement remarquable envers la participation féminine dans le domaine sportif. De même, des nations telles que la France, avec 2781 femmes, le Canada, avec 3563 femmes, et l'Australie, avec 2751 femmes, témoignent également d'une forte tradition de représentation féminine aux Jeux olympiques. Ces pays ont établi une longue tradition de soutien actif envers les athlètes féminines dans une multitude de disciplines sportives, ce qui se traduit souvent par des succès notables sur le podium. Par exemple, la Chine se distingue comme un pays avec une tradition solidement ancrée de participation féminine aux Jeux olympiques, accompagnée de performances remarquables qui se traduisent par un nombre significatif de médailles remportées par ses athlètes féminines.
...
## Analyse de la distribution des médailles par tranches de PIB par habitant
